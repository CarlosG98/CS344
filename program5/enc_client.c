...
int main(int argc, char *argv[]){
  ...
  if(strlen(keystr) < strlen(pltxt)){
    fprintf(stderr, "KEY IS SHORTER THAN PLAINTEXT.\n");
    exit(1);
  }
  //check for illegal chars
  if( check_chars(pltxt) == -1 || check_chars(keystr) == -1){
  ...
  strcat(FullMessage, "e"); //add client id to the front 
  strcat(FullMessage, pltxt); //add plaintext
...
//-------------------------SEND PLAINTEXT AND KEY---------------------------------------
  ...
  char *EncMessage = malloc(100000*sizeof(char)); //will hold encrypted message w/ termination chars

  do{
    memset(buffer, '\0', sizeof(buffer));
    charsRead = recv(socketFD, buffer, 500, 0); //recv message from server
    if(charsRead < 0) { error("CLIENT: ERROR reading from socket");}
    strcat(EncMessage, buffer); //add to message until buffer receives termination chars
  }while(strstr(EncMessage, "||") == NULL);

  char *EM = strtok(EncMessage, "||"); //strip off termination chars
  
  if( EM[strlen(EM)-1] == '-'){ //if server did not allow communication
    fprintf(stderr, "ERROR encryption client cannot connect to a decryption server.");
    free(EncMessage);
    close(socketFD);
    exit(EXIT_FAILURE);
  }else{
