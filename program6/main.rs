...
fn main() {
    ...
    for xs_elem in xs {
        handles.push(thread::spawn(move || map_data(&xs_elem)));
    }
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let mut end = partition_size; //end index for inner loop
    let mut start = 0; //start index for inner loop
    let mut i = 0; //counter
   
    //create a vector to hold vectors of ints
    let mut xs: Vec<Vec<usize>> = Vec::new();
    //create temp vector to push into xs
    let mut temp: Vec<usize> = Vec::new();
    while i < num_partitions{

        for j in start..end {
            temp.push(v[j]); //push partition_size number of elements into temp
        }
        xs.push(temp.clone()); //push cloned temp vec into xs
        temp = Vec::new(); //realloc temp for next use
        start = start + partition_size; //new start for next set of ints
        end = end + partition_size; //new end for next set of ints
        i = i +1;
    }
    
    //place any remaining data in the vectors of xs
    i = 0;
    while i < remainder {
        xs[i].push(v[(partition_size*num_partitions) + i]);
        i = i + 1;
    }
